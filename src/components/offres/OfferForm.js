...
const RenderInput = createRenderer((input, placeholder, readonly, rest) => {
    return <input className="form-control font-weight-light" {...input} placeholder={placeholder} disabled={readonly ? true : false} {...rest} />
}
)
...
class OfferForm extends Component {
    ...
    renderForAuction = () => {
        if (this.props.auction)
        {
            return (
                <>
                    <Field label="Prix d'achat immédiat par KG (Prix final)" name="end_price" component={RenderInput} placeholder="Prix final " type="number" className="form-control font-weight-light" readonly={!this.props.auction} />
                    <Field label="Garantie" name="warranty" component={RenderInput} placeholder="Garantie" type="number" className="form-control font-weight-light" min="0" max="5000" readonly={!this.props.auction} />
                </>
            );
        }
        return null;
